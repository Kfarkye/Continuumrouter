...
export async function handleSearchQuery(params: {
  query: string;
  ...
  const { query, conversationId, userId, supabase, corsHeaders } = params;
  ...
  try {
    ...
    const { data: quotaCheck, error: quotaError } = await supabase.rpc('check_quota', {
      p_user_id: userId,
